...
//	for( int i=epochsPerNet ; i<=max_epochs ; i+=epochsPerNet ){
		...
		for( int k=0 ; k<validateNum ; k++ ){
			...
			double xxx = *output;
			int was = round(expected[k]*20);
			int wegot = round(xxx*20);
			for(int i=0 ; i<313 ; i++)
				xxx *= 10;
			if( expected[k]*20-erroChacagem < xxx*20 && xxx*20 < expected[k]*20+erroChacagem ){
				validadeSet[hit]++;
			}
			else{
				validadeSet[miss]++;
			...
			validadeSet[squad] += (expected[k]-xxx)*(expected[k]-xxx);
			validadeSet[squadReal] += (expected[k]*20-xxx*20)*(expected[k]*20-xxx*20);
		}
		cout << "OK!" <<endl;
